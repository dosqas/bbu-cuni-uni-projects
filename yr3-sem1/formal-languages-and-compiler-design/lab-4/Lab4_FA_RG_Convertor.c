...
// -------------------------------------------------------------
// DEFINE FA FOR STRINGS
// -------------------------------------------------------------
// A string is defined as: " followed by letters/digits, ending with "
// Example valid strings: "abc", "a1b2c3", "Hello123"
// Invalid examples: abc", "abc, "abc def"
...
// -------------------------------------------------------------
int main() {
    // Build both automatons
    FiniteAutomaton idFA = buildIdentifierFA();
    FiniteAutomaton numFA = buildNumberFA();
    FiniteAutomaton strFA = buildStringFA();

    // Print automaton details and equivalent grammars
    printFA(&idFA, "Identifiers");
    FA_to_RG(&idFA, "Identifiers");

    printFA(&numFA, "Constants");
    FA_to_RG(&numFA, "Constants");

    printFA(&strFA, "Strings");
    FA_to_RG(&strFA, "Strings");
